...
namespace lab3_TP
{
    public static class ReadWriteFile
    {
        ...
        public static void F2WriteFile(string path, string arrData, int? D, int? M, int? sum, int? num, PersentAndNum[] PND)
        {
            FileStream fs = new FileStream(path, FileMode.OpenOrCreate);
            fs.Close();
            StreamWriter sw = new StreamWriter(path);
            ...
            if (PND == null) sw.WriteLine("-");
                ...
                foreach (var i in PND)
                {
                    ...
                    sw.WriteLine(RandomArrayGen.ArrToStringArr(i.uniqueNum));
                    sw.WriteLine(RandomArrayGen.ArrToStringArr(i.num));
                    sw.WriteLine(RandomArrayGen.ArrToStringArr(i.persent));
                }
            sw.Close();
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="path">path to F2.txt</param>
        public static void F2FileRead(string path, ref int[] arr, ref int? D, ref int? M, ref int? sum, ref int? num, ref PersentAndNum[] PND)
        {
            ...
            else
            {
                ...
                for (int i = 0; ; i++)
                {
                    ...
                    StringToArrayInFile(bufer, ref PND[i].persent);
                    bufer = sr.ReadLine();
                }
            }
            sr.Close();
